    ...
    public String sendWeChatMsg() {
        ...
        for (String opedId : configConstant.getOpenidList()) {
            ...
            try {
                ...
                //最低气温
                JSONObject minTemperature = JsonObjectUtil.packJsonObject(weatherResult.getString("tem2") + "°","#0ace3c");
                resultMap.put("minTemperature", minTemperature);
                logger.info("minTemperature:{}", minTemperature);
                //最高气温
                JSONObject maxTemperature = JsonObjectUtil.packJsonObject(weatherResult.getString("tem1") + "°","#dc1010");
                ...
                logger.info("wet:{}", wet);
                //空气质量
                JSONObject airLevel = JsonObjectUtil.packJsonObject(weatherResult.getString("air_level"), "#1f95c5");
                resultMap.put("airLevel", airLevel);
                logger.info("airLevel:{}", airLevel);
                //空气小贴士
                JSONObject airTips = JsonObjectUtil.packJsonObject(weatherResult.getString("air_tips"), "#1f95c5");
                if (weather.containsValue("雨")){
                    airTips = JsonObjectUtil.packJsonObject("要下雨啦,记得带伞哦,我的老baby", "#dc1010");
                }
                resultMap.put("airTips", airTips);
                logger.info("airTips:{}", airTips);
                //未来三天天气
